...
const Userpage = () => {
    ...
    const getPosts = async () => {
        ...
        console.log(user)
        if (user)
            await feedApi.getAllPostsByUser(user._id).then(res => {
                setPosts(res);
    ...
    const getSkills = async () => {
        ...
        console.log(user)
        if (user)
            await skillAPI.getSkillByUserId(user._id).then(res => {
                setSkills(res);
    ...
    const getJobs = async () => {
        ...
        console.log(user)
        if (user)
            await jobApi.getJobsByUser(user._id).then(res => {
                setJobs(res);
    ...
    const getEvents = async () => {
        ...
        console.log(user)
        if (user)
            await evenementApi.getEventsByUser(user._id).then(res => {
                setEvents(res);
                        ...
                        <div className="middle-sidebar-left pe-0">
                            <div className="row">
                                <div className="col-xl-12 mb-3">
                                    <ProfilecardThree user={user} isCurrentUser={isCurrentUser} activeTab={tabKey}
                                                        ...
                                                        skills.map((skill, i) => (
                                                            <div key={skill.id}>
                                                                <div key={skill.id}
                                                                    ...
                                                                    <div
                                                                        className={"card d-block border-0 shadow-xss rounded-3 overflow-hidden mb-3 bg-gold-gradiant"}>
                                                                        <div
                                                                            className="card-body d-block w-100 p-4 text-center">
                                                                            <div className="clearfix"></div>
                                                                            <h4 className="fw-700 font-xss mb-0">{skill.name} </h4>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                    ...
                                                    jobs.length > 0 ?
                                                        jobs.map((value, index) => (
                                                            ...
                                                            <div key={index}
                                                                ...
                                                                <h4 className="font-xss fw-700 text-grey-900 mb-3 pe-4">{value.title}
                                                                    <span
                                                                        className="font-xssss fw-500 text-grey-500 ms-4">({value.date})</span>
                                                                </h4>
                                                                <h5 className="font-xssss mb-2 text-grey-500 fw-600">
                                                                    <span
                                                                        className="text-grey-900 font-xssss text-dark">Location : </span> {value.address}
                                                                </h5>
                                                                <h5 className="font-xssss mb-2 text-grey-500 fw-600">
                                                                    <span
                                                                        className="text-grey-900 font-xssss text-dark">Company : </span>{value.company}
                                                                </h5>
                                                                <h5 className="font-xssss text-grey-500 fw-600 mb-3">
                                                                    <span
                                                                        className="text-grey-900 font-xssss text-dark">Salary : </span> {value.salaryRange}
                                                                </h5>
                                                            ...
                                                            <div key={index} className="col-lg-4 col-md-6 pe-2 ps-2">
                                                                <div
                                                                    ...
                                                                    <div className="card-image w-100">
                                                                        <img src={`http://localhost:8091${value.image}`}
                                                                             alt="event" className="w-100 rounded-3"/>
                                                                    </div>
